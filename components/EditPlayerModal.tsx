...
import { Game, updateGame } from "@/data/teams"; // your Game type
import { EditField } from "./EditField";
import { addGame } from "@/utils/supabase/database_functions"; // your update function
...
export function EditGameModal({
  ...
  async function submitData(event: FormEvent<HTMLFormElement>) {
    ...
    const addGame: Partial<Game> = {};

formData.forEach((value, key) => {
  const stringValue = value as string;

  if (key === "grade_levels") {
    addGame.grade_levels = stringValue.split(",").map((g) => g.trim());
  } else if (key === "game_type") {
    addGame.game_type = stringValue as Game["game_type"];
  } else if (key === "game_land") {
    addGame.game_land = stringValue as Game["game_land"];
  } else if (key === "name") {
    addGame.name = stringValue;
  } else if (key === "description") {
    addGame.description = stringValue;
  } else if (key === "math_focus") {
    addGame.math_focus = stringValue;
  } else if (key === "slug") {
    addGame.slug = stringValue;
  }
});
    ...
    const result = await updateGame(gameData.id, addGame);
